...
from .tiny_dense_training import DenseTinyTrainingConfig, load_dense_checkpoint, train_dense_tiny, write_experiment_record
...
def main() -> int:
    ...
    if args.command == "tiny-attention-gate":
        session = InferenceSession(BackendSession(RUNTIME_ROOT, "runtime", software_vulkan=args.software_vulkan), ProjectedTinyAttentionModel(259))
    ...
    if args.command == "tiny-infer":
        session = InferenceSession(BackendSession(RUNTIME_ROOT, "runtime"), ProjectedTinyAttentionModel(259))
    ...
    if args.command == "tiny-regression-series":
        ...
        comparison = load_regression_series(record)["comparison"]
    ...
    if args.command == "tiny-attention-train":
        bundle = verify_bundle(args.bundle)
        result = run_projected_training(bundle, ProjectedAttentionConfig(args.steps, args.learning_rate, args.batch_size, max_seconds=args.max_seconds, early_stop_patience=args.early_stop_patience), tokenizer=default_tokenizer(), checkpoint=args.checkpoint, artifact=args.artifact, experiment=args.experiment, resume=args.resume)
    ...
    if args.command == "tiny-dense-train":
        ...
        model, prior_steps = (None, 0)
        if args.resume:
            model, previous_metrics = load_dense_checkpoint(args.resume)
            prior_steps = int(previous_metrics["optimizer_step"])
        model, metrics = train_dense_tiny(bundle, DenseTinyTrainingConfig(args.steps, args.learning_rate, args.hidden_size, args.gradient_clip, args.memory_budget_bytes), tokenizer=tokenizer, model=model, prior_steps=prior_steps)
    ...
    if args.command == "tiny-train-dataset":
        bundle = verify_bundle(args.bundle)
        tokenizer = default_tokenizer()
        model, optimizer, metrics = train_tiny(bundle, TinyTrainingConfig(args.steps, args.learning_rate), tokenizer=tokenizer)
