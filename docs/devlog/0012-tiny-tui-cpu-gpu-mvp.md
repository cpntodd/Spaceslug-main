...
The custom runtime is now host-exposed for the validated fp32 `sgemm` parity executable and the validated attention kernel gate (`T=128, D=64`), with a CPU-first gate. Tiny GPU inference is now implemented for the fixed MVP contract (single-head H=64, V=259, padded T=128): embeddings/positions are prepared, Q/K/V and output projections use native Vulkan SGEMM, causal attention uses the new masked ABI, and the padded LM-head is sliced back to 259 logits. GPU LoRA now has a fixed-shape native train-step path for H=64, rank=4, and 1<=M<=128: GPU delta forward, dA/dB gradients, and SGD updates are each CPU-parity tested on RADV and lavapipe, and the Python session/TUI controller exposes an executed train-step result only when X and dY tensors are supplied. This is a low-rank tensor-kernel acceptance path, not yet end-to-end language-model loss/backprop training over a dataset. The inference session and TUI now expose the complete planned Tiny forward chain (`embedding_upload`, QKV SGEMM, causal softmax, output projection, lm-head) and visibly report that the chain is `not-run` until orchestration is implemented; keyboard help includes CPU inference and GPU-chain inspection actions. CLI commands `tiny-infer TOKENS...` and `tiny-gpu-chain TOKENS...` expose the same CPU baseline and explicit Vulkan chain status for non-interactive testing. `tiny-attention-gate --tokens 128 --hidden-size 64` runs the validated RX580 attention kernel gate and reports the detected RADV device and CPU-reference error. Adding `--software-vulkan` runs the same correctness gate on lavapipe; both modes pass in the current environment. The next operation is Tiny projected-attention forward parity, followed by LoRA forward/backward parity and optimizer/update parity. CPU remains authoritative.
