...
- The GPU training graph covers the implemented forward composition, causal loss/dLogits, LM-head/output/causal-attention/QKV backward, four adapter gradient paths, accumulation, graph-integrated SGD for the LM-head/output/QKV groups, and graph-integrated AdamW state/update for the LM-head and output projection, plus device-resident activation/state buffers, adapter/state readback/update, and checkpoint/restore orchestration. QKV AdamW is explicitly unavailable: it has no capability key, is not implemented, and any attempted QKV AdamW dispatch must return `-4`. Exactly two QKV AdamW implementation attempts were rejected because verification was incomplete. Standalone base-training APIs are not a bridge. This does not include dataset-batch-buffer integration: that standalone buffer is retained and GPU-processed, but it is not the training graph and `train_dataset_batch` returns `-3`.
...
- The integrated graph base-training contract is limited to LM-head SGD+AdamW, output-projection SGD+AdamW, and combined-QKV SGD for `1 <= rows <= 128`. Combined-QKV AdamW is deliberately unavailable and returns `-4`; two implementation attempts were rejected due incomplete verification. Embeddings, positions, FFN, norm, dataset training, and retained training remain unsupported; standalone caller-supplied subsets are separate APIs. The host path is bounded orchestration and parity coverage, not a GUI-complete training service, distributed trainer, multi-device trainer, or unrestricted autonomous improvement loop.
