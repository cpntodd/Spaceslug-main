...
> **Superseded where it differs from [`STAGED_ROADMAP.md`](STAGED_ROADMAP.md).** This audit was written at `spaceslug-main` `4423acd`; later commits bound graph-owned QKV AdamW (complete train/readback/update symbol set, applied to existing QKV gradients) and graph-owned embedding SGD. Where the two documents differ, the staged roadmap is authoritative. See [`0009-qkv-adamw-abi.md`](devlog/0009-qkv-adamw-abi.md) and [`0010-qkv-adamw-checkpoint.md`](devlog/0010-qkv-adamw-checkpoint.md).
...
| `vulkan-runtime` | `fd0fe95` (from `328b721`) | Graph-owned embedding SGD integration and CPU-parity test under `src/` and `tests/` |

The current runtime facts are: the integrated Tiny graph exposes fixed-window SGD for the LM-head, output projection, combined QKV, and **token embeddings** (graph-owned embedding SGD integrated by runtime `328b721`, verified by CPU-parity test at `fd0fe95`); every integrated SGD group accepts exactly `1 <= rows <= 128`; LM-head, output-projection, and combined-QKV AdamW are graph-owned and capability-gated (each returns `-4` when its complete train/readback/update symbol set is absent); positions, FFN, and norm remain unsupported; device-resident dataset LM-head SGD is optionally supported for at most 32 windows of length <=128 via BatchBuffer, while full-dataset and all-parameter training remain unsupported; retained training remains unsupported; standalone FP32 subset APIs remain distinct and caller-supplied, and standalone embedding training remains separate rather than a fallback for graph-owned embedding SGD; the `dataset_batch_buffer` is only standalone staging and `ForwardResourceGraph::train_dataset_batch(...)` returns `-3`; `train_dataset_batch_full(...)` validates bounded inputs but returns `-4` until complete graph integration; the FP16 Tiny constructor is an evaluation/storage path that widens frozen binary16 inputs to the unchanged FP32 graph; ranks 4 and 8 are supported; fixed retained forward+loss is exposed through the native `spaceslug_tiny_forward_loss_fixed_retained` ABI and Python `execute_tiny_fixed_retained_loss` / `PersistentTinyTrainer.fixed_forward_loss` binding; and full training uses ordinary submissions. These are implementation contracts, not separate acceptance releases.
...
- The GPU training graph covers the implemented forward composition, causal loss/dLogits, LM-head/output/causal-attention/QKV backward, four adapter gradient paths, accumulation, graph-integrated SGD for the LM-head/output/QKV/**embedding** groups, and graph-integrated AdamW state/update for the LM-head, output projection, and combined QKV, plus device-resident activation/state buffers, adapter/state readback/update, and checkpoint/restore orchestration. Graph-owned embedding SGD is sparse FP32 SGD for the current host-staged window; it is distinct from the standalone caller-supplied embedding API and does not add positions, FFN, or norm. QKV AdamW is capability-gated: it is available when the complete QKV train/readback/update symbol set is exported, applies to existing QKV gradients without recompute or double update, and returns `-4` when the symbol set is absent. Standalone base-training APIs are not a bridge. This does not include dataset-batch-buffer integration: that standalone buffer is retained and GPU-processed, but it is not the training graph; `train_dataset_batch` returns `-3`, and the full-batch probe reports `unsupported` / `-4` with an explicit fallback reason.
- The host exposes explicit capability and fallback/status metadata. Every native training step must confirm `gpu_execution`; CPU remains authoritative when a shape, rank, operation, device, or runtime prerequisite is outside the GPU contract.
...
- The runtime's Tiny capability metadata names rank 4/rank 8 LoRA, integrated LM-head/output/QKV/**embedding** SGD for `rows <= 128`, and integrated LM-head/output/QKV AdamW support (capability-gated on the complete symbol set); positions, FFN, and norm are explicitly unsupported. Standalone embedding APIs remain separate and are not used as fallback. Dataset training and retained training are also unsupported. This does not mean every combination is one retained graph or a complete end-to-end product trainer.
...
- This is not a claim of full-graph retained training. The exact fixed-shape forward+loss subset is retained, but backward, LoRA gradient/adapter update, and optimizer/update are not retained capabilities and remain unsupported in the retained path; general training methods use bounded normal submissions. A fixed full-training retained subset was not accepted because its synchronization, mutable optimizer controls, and rank-dependent staging were not verified. Integrated graph SGD/AdamW support is a normal-submission capability, not retained-command training; AdamW for output and QKV is capability-gated on the complete symbol set.
...
- The integrated graph base-training contract is limited to LM-head SGD+AdamW, output-projection SGD+AdamW, combined-QKV SGD+AdamW, and graph-owned token-embedding sparse SGD for `1 <= rows <= 128`. AdamW for each group is capability-gated on the complete symbol set and returns `-4` when absent; QKV AdamW applies to existing QKV gradients without recompute or double update. Positions, FFN, norm, dataset training, and retained training remain unsupported; standalone caller-supplied subsets—including standalone embedding training—are separate APIs and never fall back into the graph-owned path. The host path is bounded orchestration and parity coverage, not a GUI-complete training service, distributed trainer, multi-device trainer, or unrestricted autonomous improvement loop.
