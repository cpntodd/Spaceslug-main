...
The optional `spaceslug_tiny_forward_readback_graph_dstate` ABI is exposed by `BackendSession.readback_tiny_graph_dstate`. It accepts equal token/target/mask windows with `1 <= rows <= 128`, returns graph-produced FP32 `dstate` rows with hidden size 64 plus zero-padded capacity rows through 128, and is readback-only. Separately, `train_tiny_graph_embeddings_sgd` performs graph-owned sparse embedding SGD in the same bounded backward submission; capability/status metadata distinguishes these two paths, and absent symbols remain unsupported without fallback.
...
These are **implemented standalone** native ABIs: LM-head, output projection, and combined QKV remain independent caller-supplied composition primitives, not a complete transformer trainer. Separately, runtime `328b721` integrates graph-owned sparse FP32 SGD for token embeddings, with CPU-parity verification at `fd0fe95`; this path is bound independently and does not use the standalone embedding API as fallback. Graph-owned positional tables now support bounded FP32 SGD through `train_tiny_graph_positions_sgd`. Graph-owned base checkpoints are schema v2 and include embeddings and positions; these SGD groups have no moment state. AdamW positional state, feed-forward weights, normalization, retained training, and dataset-integrated full training remain unsupported. The runtime graph separately owns the Tiny LM-head parameter and exposes FP32 import/readback. When the runtime exports the complete AdamW symbol set, ctypes binds graph-owned LM-head AdamW training plus `m`/`v`/step readback and update; the same conditional binding applies to graph-owned output projection AdamW (`tiny_graph_integrated_output_adamw`) and combined QKV AdamW (`tiny_graph_integrated_qkv_adamw`), which applies AdamW to existing QKV gradients without recompute or double update. If required symbols are absent, the binding reports **`-4`**. This remains a fixed 1..128 host-staged token window: it is not dataset-device training, retained-command training, or full-base training. Graph ownership and integrated AdamW must not be confused with the standalone API. `full_base_training` is false; do not interpret these bounded groups as full base training. CPU remains authoritative for the integrated training path, and tests assert these boundaries.
