...
Tiny projected-attention GPU LoRA training is implemented for the fixed MVP contract (single-head H=64, V=259, padded T=128, rank=4): LoRA-aware Q/K/V/output forward composition, padded-vocabulary causal loss and dLogits, LM-head/output/causal-attention backward, four-adapter dA/dB gradients, and GPU SGD updates are composed and CPU-parity tested on both RADV and lavapipe. A persistent native tensor session keeps LoRA A/B and tensor-step buffers device-resident across repeated submissions, with reusable pipelines, ExecEngine timeline submission, adapter readback, and checkpoint/resume integration. The token-derived full LM graph remains transient and unsupported as a persistent graph; gradient accumulation, AdamW, and dataset-scale training are also unsupported. CPU remains authoritative outside this bounded experimental path.
