...
Tiny projected-attention GPU LoRA training is implemented for the fixed MVP contract (single-head H=64, V=259, padded T=128, rank=4): LoRA-aware Q/K/V/output forward composition, padded-vocabulary causal loss and dLogits, LM-head/output/causal-attention/QKV backward, four-adapter dA/dB gradients, GPU SGD, persistent device-resident activations, reusable ExecEngine submissions, and adapter checkpoint/restore. The mutable-input immutable Tiny command-reuse path now has a verified native prototype. Production full-graph integration remains explicitly bounded to fp32 H=64/V=259/Vp=320/T<=128, rank=4 or rank=8, frozen base weights, and bounded host/Python orchestration; CPU remains authoritative outside this experimental path. Graph-owned SGD is available for token embeddings, the LM-head, output projection, and combined Q/K/V for 1–128 rows; graph-owned AdamW is available for the LM-head, output projection, and combined Q/K/V. Positions, FFN, and normalization remain unsupported, and FP16 is a storage format only.

Phase 1 also ships the committed desktop/API surface: a native Tkinter/ttk shell (`spaceslug desktop`) with five tabs (Home, Datasets, Build & Train, Interact, Local API), the headless workspace/ingestion service with approval-gated URL import and SearXNG search, and a loopback-only OpenAI-compatible HTTP service (`GET /health`, `GET /v1/models`, `POST /v1/chat/completions`, non-streaming). Local dataset sources are `.txt`/`.md`/`.jsonl` plus `.pdf`; PDF text is extracted by the local `pdftotext` executable (from `poppler-utils`) and fails closed with `PDFToolMissingError` when the tool is absent, or `PDFExtractionError` when it exits non-zero or yields no text. See [`docs/USER_GUIDE_PHASE1.md`](docs/USER_GUIDE_PHASE1.md).
...
PYTHONPATH=python python3 -m spaceslug.cli desktop   # Phase 1 Tkinter desktop shell
