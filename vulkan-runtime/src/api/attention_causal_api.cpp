...
struct PushConstants {
    std::uint32_t t;
    std::uint32_t d;
    std::uint32_t actual_tokens;
};
} // namespace

extern "C" int spaceslug_attention_causal(float const* q,
                                          float const* k,
                                          float const* v,
                                          float* output,
                                          std::uint32_t t,
                                          std::uint32_t d) {
    if (q == nullptr || k == nullptr || v == nullptr || output == nullptr || t == 0 ||
        t > SPACESLUG_ATTENTION_CAUSAL_T || d != SPACESLUG_ATTENTION_CAUSAL_D)
        return 1;

    try {
        constexpr std::size_t bytes = static_cast<std::size_t>(SPACESLUG_ATTENTION_CAUSAL_FLOATS) * sizeof(float);
        ...
        auto blob = vulkan_runtime::shaders::get("attention_causal.spv");
        if (blob.data == nullptr || blob.size == 0)
            throw std::runtime_error("attention_causal.spv not embedded");
        ...
        PushConstants constants{SPACESLUG_ATTENTION_CAUSAL_T, d, t};
        cmd.pushConstants(pipelineLayout, vk::ShaderStageFlagBits::eCompute, 0, sizeof(constants), &constants);
        cmd.dispatch(SPACESLUG_ATTENTION_CAUSAL_T / 16, 1, 1);
