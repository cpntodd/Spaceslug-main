...
class LoraReferenceTest(unittest.TestCase):
    ...
    def test_state_and_shapes(self):
        adapter=TinyLoRAAdapter(); state=adapter.state_dict()
        self.assertEqual(state["rank"],4); self.assertEqual(len(state["matrices"]["query"]["A"]),64); self.assertEqual(len(state["matrices"]["query"]["B"]),4)
