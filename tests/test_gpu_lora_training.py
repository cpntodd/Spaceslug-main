...
class GpuLoraTrainingStateTest(unittest.TestCase):
    ...
    def test_persistent_graph_boundary_is_explicit(self):
        ...
        self.assertEqual(boundary["production_status"], "bounded")
        self.assertTrue(boundary["immutable_command_buffer_reuse_prototype"])
    ...
    def test_capability_keeps_unimplemented_paths_explicit(self):
        ...
        self.assertTrue(capability["fixed_forward_retained_only"])
        self.assertTrue(capability["fixed_forward_loss_retained"])
        self.assertFalse(capability["retained_backward"])
        self.assertFalse(capability["retained_lora"])
        self.assertFalse(capability["retained_optimizer"])
        self.assertEqual(capability["training_submission"], "normal-submit")
        self.assertEqual(capability["dataset_batch_buffer"], "standalone-not-training")
        self.assertEqual(capability["dataset_batch_training_return_code"], -3)
        ...
        self.assertTrue(capability["fp16_storage_evaluation"])
    ...
    def test_training_plan_lists_all_gpu_stages_and_boundaries(self):
        ...
        self.assertEqual(plan["production_status"], "bounded")
